...
if (isset($_POST['submit'])) {
    $attendanceNewstatus = $_POST['status'];
    $id = 1; 

    $stmt = $conn->prepare("UPDATE attendance SET status = ? WHERE id = ?");
    $stmt->bind_param("si", $attendanceNewstatus, $id);
    $stmt->execute();

    if ($result) {
        echo "<script>alert('Status Updated Successfully!'); window.location.href = 'attendance_status.php';</script>";
    } else {
...
<body>
   ...
   <div class="status-card">
                <div>
                    ...
                    <div class="status-value text-<?php echo $status === 'attendance' ? 'success' : 'danger'; ?>">
                        <?php echo ucfirst($status); ?>
                    </div>
                    <div class="timestamp">Last updated: <?php echo date('Y-m-d H:i:s'); ?></div>
                </div>
                <i class="bi bi-power" style="font-size: 2rem; color: <?php echo $status === 'attendance' ? 'var(--success-color)' : 'var(--danger-color)'; ?>"></i>
            </div>
                ...
                <form method="POST">
                    <div class="mb-4">
                        ...
                        <select class="form-select" name="status" id="status" required>
                            <option value="attendance" <?php echo $status === 'attendance' ? 'selected' : ''; ?>>Attendance - The student can now mark their attendance</option>
                            <option value="register" <?php echo $status === 'register' ? 'selected' : ''; ?>>Register - The student is now able to register a seat</option>
                        </select>
